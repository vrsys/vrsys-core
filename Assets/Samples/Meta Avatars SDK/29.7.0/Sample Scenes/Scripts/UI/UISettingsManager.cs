...
public class UISettingsManager : MonoBehaviour
{
    ...
    private void AddPresetQualitiesToDropdown()
    {
        ...
        if (presetSelection == null)
        {
            OvrAvatarLog.LogError(
                "UISettingsManager::AddPresetQualitiesToDropdown : no preset selection scriptable object found.",
                logScope);
            return;
        }

        if (presetSelection.qualitySelection == null)
        {
            OvrAvatarLog.LogError(
                "UISettingsManager::AddPresetQualitiesToDropdown : null PresetHelper quality selection", logScope);
            return;
        }

        for (var i = 0; i < presetSelection.qualitySelection.Length; i++)
        {
            if (presetSelection.qualitySelection[i])
            {
                optionData.Add(
                    new Dropdown.OptionData(Enum.GetName(typeof(CAPI.ovrAvatar2EntityQuality), i)));
            }
        }
    ...
    private void AddPresetsToDropdown()
    {
        ...
        {
            OvrAvatarLog.LogError(
                "UISettingsManager::AddPresetsToDropdown : null PresetHelper avatar selection", logScope);
            return;
        }
