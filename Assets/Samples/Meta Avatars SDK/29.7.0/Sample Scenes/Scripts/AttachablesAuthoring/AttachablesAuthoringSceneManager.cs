...
public class AttachablesAuthoringSceneManager : MonoBehaviour
{
    ...
    private void OnBaseScaleSliderChange()
    {
        var baseScale = socketCreationInfo.hasBaseScaleMod ?
            new Vector3(ScaleSlider(xBaseScaleSlider.value), ScaleSlider(yBaseScaleSlider.value),
                ScaleSlider(zBaseScaleSlider.value)) :
            Vector3.one;
    ...
    private void UpdateText()
    {
        createSocketText.text = $"CreateSocket(\n" +
                                ...
                                $"\tbaseScale: " + (socketCreationInfo.hasBaseScaleMod ? $"new Vector3({ScaleSlider(xBaseScaleSlider.value)}f, {ScaleSlider(yBaseScaleSlider.value)}f, {ScaleSlider(zBaseScaleSlider.value)}f), \n" : "null, \n") +
