...
namespace Oculus.Avatar2
{
    public class MecanimLegsAnimationController : MonoBehaviour, IUIControllerInterface
    {
        ...
        private float _crouchStartThreshold = -0.1f;
        ...
        void UpdateState()
        {
            ...
            else
            {
                ...
                if (_enableCrouching && _currentHeadHeight < crouchStartThreshold)
                {
        ...
        protected bool PollInitialPosition()
        {
            ...
            // We can't get the initial head position until the tracking pose is valid on the Avatar.
            if (!_entity.TrackingPoseValid)
            {
                return false;
            }

            // Get the initial head height, and calculate where the crouch start threshold should be.
            var headPositionVector = GetHeadPosition();
            if (headPositionVector is null)
            {
